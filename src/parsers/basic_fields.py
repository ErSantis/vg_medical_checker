...
def parse_basic_fields(text: str) -> BasicFieldsDetailed:
    ...
    def extract_field(field_name: str, post_clean=None, long_value=False) -> FieldValue:
        ...
        for ln in lines:
            ...
            if exact:
                val = exact if not long_value else exact  # (podríamos extender a varias líneas si fuera necesario)
                if post_clean:
                    val = post_clean(val)
